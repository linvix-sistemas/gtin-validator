...
import { gtinLengthMap } from "../modules/generateRandomGTIN";
...
test.each([
    ...
    (gtinType, numberOfGTINsToGenerate, guaranteeOnlyUniqueGTINs) => {
        const randomGTINs = generateArrayOfRandomGTINs(
            //@ts-ignore
            gtinType,
...
test.each([
    ...
    ["GTIN-14", 505, false],
    ["GTIN-8", 123456, true],
])("generateArrayOfRandomGTINs valid tests", (gtinType, numberOfGTINsToGenerate, guaranteeOnlyUniqueGTINs) => {
    //@ts-ignore
    const randomGTINs = generateArrayOfRandomGTINs(
        //@ts-ignore
        gtinType,
...
])("generateArrayOfRandomGTINs invalid tests", (gtinType, numberOfGTINsToGenerate, guaranteeOnlyUniqueGTINs, error) => {
    //@ts-ignore
    expect(() =>
